...
export default function MortgageTab() {
  const [mortgageSubTab, setMortgageSubTab] = useState('calculator');

  // Обработка URL параметров для открытия нужной подвкладки
  useEffect(() => {
    const params = new URLSearchParams(window.location.search);
    const subtab = params.get('subtab');
    if (subtab && ['calculator', 'programs', 'comparison'].includes(subtab)) {
      setMortgageSubTab(subtab);
    }
  }, []);

  return (
    <div className="space-y-4">
      <Tabs value={mortgageSubTab} onValueChange={setMortgageSubTab}>
        <TabsList className="bg-white/80 backdrop-blur-sm p-1 rounded-lg shadow-sm grid grid-cols-3 gap-1">
          <TabsTrigger 
            value="calculator" 
